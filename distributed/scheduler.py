...
class ClientState:
    ...
    def dump_stats(self):
        print('Lets dumps the stats')
        with open(os.path.join('/tmp/dask', f'{self._client_key}.json'), 'w') as fd:
            #json.dump(self._stats, fd) #.replace("'", '"'))
    ...
    * **task_duration:** ``{key-prefix: time}``
    ...
    def transition_waiting_processing(self, key):
        try:
            ...
            ts._task_submit_time = datetime.now().timestamp()
...
class Scheduler(SchedulerState, ServerNode):
    ...
    * **task_duration:** ``{key-prefix: time}``
    ...
    def handle_task_finished(self, key=None, worker=None, **msg):
        ...
        submit_ts = self.tasks[key].get_ts()
        print('inside handle_task_finished {submit_ts}')
        self.clients[str(cs)]._stats[key] = {"worker": str(worker), "msg": msg, 'submit_ts': submit_ts}
